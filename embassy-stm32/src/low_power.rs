...
mod platform {
    ...
    pub fn enter_stop(_cs: CriticalSection, stop_mode: StopMode) -> Result<(), ()> {
        ...
        #[cfg(stm32l0)]
        {
            ...
            // if ulp / fwu shall be set, this is application specific and therefore up to the prorammer.
            // this flags can be set in the application specifif pre-/post_wfi_cb
...
/// returns true, if stop was entered, false if not
...
{
    *PRE_WFI_CB.borrow(cs).borrow_mut() = pre_wfi_cb;
    *POST_WFI_CB.borrow(cs).borrow_mut() = post_wfi_cb;
}
...
pub unsafe fn sleep(cs: CriticalSection) {
    ...
    #[cfg(feature = "low-power-idle-callbacks")]
    if let Some(cb) = *PRE_WFI_CB.borrow(cs).borrow()
    {
        cb(cs, mode_is_stop);
    }
